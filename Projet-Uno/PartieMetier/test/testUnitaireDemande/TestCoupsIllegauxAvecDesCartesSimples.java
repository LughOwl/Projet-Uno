...
public class TestCoupsIllegauxAvecDesCartesSimples {
    ...
    @Test
    public void TestUneCarteIllegale(){
    ...
    @Test
    public void TestUnJoueurQuiPoseDeuxCartesLegalesDeSuite() throws UnoException, PunishmentException {
        ...
        charles.playCard(blue9);
        assertEquals(2, charles.getHand().size());

        assertThrows(PunishmentException.class, () -> charles.playCard(blue7));
        assertEquals(2, charles.getHand().size());
        assertTrue(charles.getHand().contains(blue7));
    }

    @Test
    public void TestUnJoueurQuiFinitSonTourSansRienFaire(){
        assertThrows(UnoException.class, () -> alice.finishTurn());
        assertEquals(3, alice.getHand().size());
    }

    @Test
    public void TestUnJoueurQuiJouePuisPioche() throws UnoException, PunishmentException {
        alice.playCard(green2);
        assertThrows(PunishmentException.class, () -> alice.drawCard());
        assertEquals(2, alice.getHand().size());
        assertEquals(yellow6, game.getDrawPile().getTopCard());
    }
}
